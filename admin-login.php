...
// Fuerza configuración segura de sesión
...
if (isset($_POST['login']) && $_SESSION['bloqueo'] <= time()) {
    ...
    if ($usuario === "" || $contrasena === "") {
        ...
        $stmt = $conn->prepare("SELECT id_admin, usuario_admin, contrasena FROM administradores WHERE usuario_admin = ?");
        ...
        if ($resultado->num_rows > 0) {
            ...
            // Solo password_verify (seguro)
            if (password_verify($contrasena, $admin['contrasena'])) {

                session_regenerate_id(true); 
                $_SESSION['usuario_admin'] = $admin['usuario_admin'];

                $upd = $conn->prepare("UPDATE administradores SET ultimo_login = NOW() WHERE id_admin = ?");
...
.btn-gamer:hover { transform: scale(1.05); box-shadow: 0 0 15px #00ff6a; }
...
<div id="temporizador">Tiempo restante: <span id="tiempo"><?= htmlspecialchars($tiempoRestante) ?></span>s</div>
...
<div class="login-container">
  ...
  <form method="POST">
    ...
    <button type="submit" name="login" class="btn btn-gamer w-100 mt-3" <?= $tiempoRestante>0?'disabled':'' ?>>
